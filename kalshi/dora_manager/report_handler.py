"""
Report Handler for Dora Manager

Handles the 'report' execution mode (every 3 hours):
1. Fetches trading data from DynamoDB
2. Calculates P&L and statistics for the window
3. Flags and disables underperforming markets
4. Sends email report via SES
"""
import logging
import os
...
def handle_report(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Handle report mode execution.

    Args:
        event: Lambda event with configuration overrides
    ...
    logger.info(f"Starting Report Mode - Environment: {environment}, Region: {region}")
    ...
    try:
        ...
        logger.info(f"Report mode completed successfully")
        return response

    except Exception as e:
        logger.error(f"Error in report mode: {str(e)}", exc_info=True)
        return {
            'statusCode': 500,
            'body': {
                'success': False,
